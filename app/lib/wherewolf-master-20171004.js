(function() {
  ...
  function _inside(point, feature) {
    ...
    if (feature.geometry.type === 'Point') {
      return (
        point[0] == feature.geometry.coordinates[0] &&
        point[1] == feature.geometry.coordinates[1]
      );
    }
    ...
    if (feature.geometry.type === 'Polygon') {
      return (
        inRing(feature.geometry.coordinates[0]) &&
        !feature.geometry.coordinates.slice(1).some(inRing)
      );
    }
    ...
    for (var i = 0, l = feature.geometry.coordinates.length; i < l; i++) {
      if (
        inRing(feature.geometry.coordinates[i][0]) &&
        !feature.geometry.coordinates[i].slice(1).some(inRing)
      ) {
        return true;
      }
    }
  ...
  function _convertTopo(collection, key) {
    var converted, features;

    //Check that topojson exists
    if (!tj) {
      throw new Error(
        'You must include the TopoJSON client library (https://github.com/mbostock/topojson) if you\'re using a TopoJSON file.'
      );
    }
    ...
    if (typeof key !== 'string') {
      ...
      if (keys.length == 1) {
        ...
        throw new Error(
          'You supplied a topology with multiple objects: ' +
            JSON.stringify(keys) +
            '.  Specify an object to add, or use .addAll().'
        );
      }
      else {
      ...
      throw new Error(
        'The key \'' + key + '\' was not found in your TopoJSON object.'
      );
    }
  ...
  function _getBBox(feature) {
    ...
    if (feature.geometry.type === 'Point') {
      return [feature.geometry.coordinates, feature.geometry.coordinates];
    }
    ...
    var outer =
      feature.geometry.type === 'Polygon'
        ...
        : feature.geometry.coordinates.map(function(f) {
          return f[0];
        });
    ...
    outer.forEach(function(polygon) {
      polygon.forEach(function(point) {
        bounds = [
          [Math.min(point[0], bounds[0][0]), Math.min(point[1], bounds[0][1])],
          [Math.max(point[0], bounds[1][0]), Math.max(point[1], bounds[1][1])]
        ];
      });
    });
  ...
  function _winding(point, vs) {
    ...
    function dir(v1, v2, p) {
      return (
        (v2[0] - v1[0]) * (p[1] - v1[1]) - (p[0] - v1[0]) * (v2[1] - v1[1])
      );
    }
    ...
    if (
      vs[vs.length - 1][0] !== vs[0][0] ||
      vs[vs.length - 1][1] !== vs[0][1]
    ) {
      vs = vs.slice(0);
      vs.push(vs[0]);
    }
